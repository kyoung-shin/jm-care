...
export async function GET() {
  try {
    ...
    const [students, instructors, pendingCount, recentCounselings, recentPending] = await Promise.all([
      ...
      prisma.pendingUser.count({ where: { status: 'PENDING', requestedRole: 'DIRECTOR' } }),
      prisma.counseling.findMany({ take: 5, orderBy: { createdAt: 'desc' }, include: { student: true } }),
      prisma.pendingUser.findMany({ take: 5, where: { status: 'PENDING', requestedRole: 'DIRECTOR' }, orderBy: { createdAt: 'desc' } }),
    ]);
